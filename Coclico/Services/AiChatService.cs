...
public sealed class AiChatService : IAiService, IDisposable
{
    ...
    private sealed class ChatCtx(LLamaContext ctx) : IDisposable
    {
        ...
        public void Dispose() { try { Context.Dispose(); } catch { } }
    }

    private volatile ChatCtx? _chatCtx;
    private readonly SemaphoreSlim _chatSem = new(1, 1);

    private volatile ChatCtx? _engineCtx;
    ...
    public AiChatService(SettingsService settings)
    {
        _settings = settings;
    }
        ...
        try
        {
            ...
            foreach (var gpuLayers in GpuAttempts)
            {
                try
                {
                    ...
                    _initialized = true;
                    LoggingService.LogInfo($"[AiChatService] Modèle chargé — GPU layers: {gpuLayers}");
                    ResetIdleTimer();
                    return;
                }
    ...
    private Task LoadModelAsync(int gpuLayers, CancellationToken ct)
    {
        ...
        return Task.Run(() =>
        {
            ...
            var engineParams = BuildEngineContextParams(gpuLayers);
            Interlocked.Exchange(ref _engineCtx, new ChatCtx(_model.CreateContext(engineParams)))?.Dispose();
        }, ct);
    }

    private static ModelParams BuildEngineContextParams(int gpuLayers) => new(ModelPath)
    {
        ContextSize = 512,
        GpuLayerCount = gpuLayers,
        Threads = Math.Max(1, OptimalThreads / 2),
        BatchSize = 128,
        UseMemorymap = true,
        FlashAttention = true,
    };
        ...
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        ...
        ctx = _chatCtx;
        if (ctx == null) { _chatSem.Release(); yield break; }
        ...
        try
        {
            await foreach (var token in ctx.Executor.InferAsync(prompt, inferenceParams, ct).ConfigureAwait(false))
                yield return token;
        }
        finally
        {
    ...
    public async IAsyncEnumerable<string> SendSystemPromptAsync(
        string systemPrompt,
        [EnumeratorCancellation] CancellationToken ct = default)
    {
        if (!_initialized) await InitializeAsync(ct).ConfigureAwait(false);

        await _engineSem.WaitAsync(ct).ConfigureAwait(false);
        try
        {
            ResetEngineContext();

            var engineCtx = _engineCtx;
            if (engineCtx == null) yield break;
            ...
            {
                MaxTokens = 256,
                AntiPrompts = new[] { "<|end|>", "<|user|>", "<|endoftext|>" },
                SamplingPipeline = new DefaultSamplingPipeline
                {
                    Temperature = 0.1f,
                    TopP = 0.95f,
            ...
            await foreach (var token in engineCtx.Executor.InferAsync(prompt, inferenceParams, ct).ConfigureAwait(false))
                yield return token;
        }
        finally
        {
    ...
    private void ResetContextOnly()
    {
        ...
        var newCtx = new ChatCtx(_model.CreateContext(BuildModelParams(_activeGpuLayers)));
        Interlocked.Exchange(ref _chatCtx, newCtx)?.Dispose();
    }

    private void ResetEngineContext()
    {
        if (_model == null) return;
        try
        {
            var newCtx = new ChatCtx(_model.CreateContext(BuildEngineContextParams(_activeGpuLayers)));
            Interlocked.Exchange(ref _engineCtx, newCtx)?.Dispose();
        }
        catch (Exception ex)
        {
            LoggingService.LogException(ex, "AiChatService.ResetEngineContext");
        }
    }

    private void ResetIdleTimer()
    {
        var minutes = _settings.Settings.AiIdleTimeoutMinutes;
        if (minutes <= 0)
        {
    ...
    private void OnIdleTimeout(object? state)
    {
        ...
        {
            Interlocked.Exchange(ref _unloading, 0);
        }
    }
    ...
    private async Task UnloadModelAsync()
    {
        ...
        try
        {
            ...
            try
            {
                await _engineSem.WaitAsync().ConfigureAwait(false);
                try
                {
                    DisposeModelInternal();
                    _initialized = false;
                    _firstMessage = true;
                    LoggingService.LogInfo("[AiChatService] Modèle déchargé après inactivité — ~2.5 Go RAM libérés");
                }
                finally { _engineSem.Release(); }
            }
    ...
    {
        Interlocked.Exchange(ref _engineCtx, null)?.Dispose();
