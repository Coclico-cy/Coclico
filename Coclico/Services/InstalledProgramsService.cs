...
private List<ProgramInfo>? _memoryCache;
    ...
    public List<string> GetCategories()
    {
        ...
        catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.GetCategories"); }
    ...
    public void SaveCategories(List<string> categories)
    {
        try
        {
            ...
            string content = JsonSerializer.Serialize(categories.Take(255).ToList());
    ...
    public List<FilterGroup> GetFilterGroups()
    {
        try
        {
            if (File.Exists(FilterGroupsPath))
            {
                ...
                var groups = JsonSerializer.Deserialize<List<FilterGroup>>(content) ?? GetDefaultFilterGroups();
                bool dirty = false;
        ...
        catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.GetFilterGroups"); }
    ...
    public void SaveFilterGroups(List<FilterGroup> groups)
    {
        try
        {
            ...
            string content = JsonSerializer.Serialize(toSave);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!forceRefresh)
        {
            try
            {
                ...
                var cached = await ServiceContainer.GetRequired<ICacheService>().GetAsync<List<ProgramInfo>>(cacheKey).ConfigureAwait(false);
        ...
        var scans = new Func<CancellationToken, List<ProgramInfo>>[]
        {
            ScanEpicGames,
            ScanSteam,
            ScanGOG,
        ...
        try
        {
            ...
            await ServiceContainer.GetRequired<ICacheService>().SetAsync(cacheKey, finalizedList, ttl).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    private static List<ProgramInfo> ScanEA(CancellationToken cancellationToken)
    {
        ...
        catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.ScanEA"); }
        return list;
    }

    private static List<ProgramInfo> ScanRockstar(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
        try
        {
        ...
        catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.ScanRockstar"); }
        return list;
    }
    ...
    private static List<ProgramInfo> ScanStartMenu(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
        var paths = new[] {
        ...
        foreach (var rootPath in paths)
        {
            ...
            foreach (var shortcutPath in shortcuts)
            {
                ...
                catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.ScanStartMenu.ReadShortcut"); }
            }
        }
        return list;
    }

    private static List<ProgramInfo> ScanRegistryUninstall(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
        ...
        foreach (var root in rootKeys)
        {
            foreach (var path in registryPaths)
            {
                ...
                catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.ScanRegistryUninstall.ReadSubkey"); }
            }
        }
        return list;
    }

    private static List<ProgramInfo> ScanEpicGames(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
        ...
        foreach (var file in Directory.GetFiles(epicManifests, "*.item"))
        {
            ...
            catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.ScanEpicGames.ReadManifest"); }
        }
        return list;
    }

    private static List<ProgramInfo> ScanSteam(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
        ...
        foreach (var file in Directory.GetFiles(appsPath, "appmanifest_*.acf"))
        {
            ...
            catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.ScanSteam.ReadManifest"); }
        }
        return list;
    }

    private static List<ProgramInfo> ScanGOG(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
    ...
    private static List<ProgramInfo> ScanUbisoft(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
    ...
    private static List<ProgramInfo> ScanUWPApps(CancellationToken cancellationToken)
    {
        var list = new List<ProgramInfo>();
        try
        {
    ...
    private void SaveCustomAppDataByKey(string key, string? name, string? category, string cacheLookupPath)
    {
        try
        {
            ...
            try { ServiceContainer.GetRequired<ICacheService>().Invalidate("programs_scan_v1"); } catch (Exception ex2) { LoggingService.LogException(ex2, "InstalledProgramsService.SaveCustomAppDataByKey.InvalidateCache"); }
        }
    ...
    public void AddManualApplication(ProgramInfo app)
    {
        ...
        if (!apps.Any(a => a.ExePath.Equals(app.ExePath, StringComparison.OrdinalIgnoreCase)))
        {
            ...
            _memoryCache = null;
            try { ServiceContainer.GetRequired<ICacheService>().Invalidate("programs_scan_v1"); } catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.AddManualApplication.InvalidateCache"); }
    ...
    private static List<ProgramInfo> LoadManualApplications()
    {
        try
        {
            if (File.Exists(ManualAppsPath))
            {
                ...
                return JsonSerializer.Deserialize<List<ProgramInfo>>(content) ?? [];
            }
        }
        catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.LoadManualApplications"); }
        return [];
    }
    ...
    private static void SaveManualApplications(List<ProgramInfo> apps)
    {
        try
        {
            ...
            if (!Directory.Exists(dir)) Directory.CreateDirectory(dir);
            string content = JsonSerializer.Serialize(apps);
    ...
    private static string? FindMainExecutable(string folderPath)
    {
        ...
        catch (Exception ex) { LoggingService.LogException(ex, "InstalledProgramsService.FindMainExecutable"); return null; }
    }
