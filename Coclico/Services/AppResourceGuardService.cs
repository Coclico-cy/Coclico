...
{
    public sealed class AppResourceGuardService : IDisposable
    {
        private static readonly Lazy<AppResourceGuardService> _lazy =
            new(() => new AppResourceGuardService(), LazyThreadSafetyMode.ExecutionAndPublication);
        public static AppResourceGuardService Instance => _lazy.Value;
        ...
        private AppResourceGuardService()
        {
        ...
        public void Start()
        {
            ...
            _subscription = Observable
                ...
                {
                    LoggingService.LogException(ex, "AppResourceGuardService.Tick");
        ...
        private void ApplyPressurePolicy(MemoryCleanerService.RamInfo sys)
        {
            ...
            switch (CurrentPressure)
            {
                case PressureLevel.Critical:
                    ...
                    if (_consecutiveHigh >= HighStreakForGc)
                    {
                        AggressiveGc();
                        _consecutiveHigh = 0;
                    }
                ...
                case PressureLevel.High:
                    _consecutiveHigh++;
                    if (_consecutiveHigh >= HighStreakForGc * 2)
                    {
                ...
                case PressureLevel.Elevated:
                    _consecutiveHigh = Math.Max(0, _consecutiveHigh - 1);
