...
namespace Coclico
{
    ...
    {
        private Coclico.Services.TrayService? _trayService;
        private readonly UserAccountService _userService;
        private readonly ProcessWatcherService _watcher = ProcessWatcherService.Instance;

        private DashboardView? _dashboardView;
        private ProgramsView? _programsView;
        private FlowChainsView? _flowChainsView;
        ...
        private readonly KeyboardShortcutsService _hotkeyService = KeyboardShortcutsService.Instance;

        private IEnumerable<System.Windows.Controls.TextBlock> NavLabels => new[]
        {
            NavHome_Lbl, NavPrograms_Lbl, NavFlowChains_Lbl,
            NavInstaller_Lbl, NavCleaning_Lbl, NavScanner_Lbl,
            NavRamCleaner_Lbl,
            NavSettings_Lbl, NavHelp_Lbl, NavAi_Lbl
        };

        private System.Windows.Controls.RadioButton[] NavButtons => new[]
        {
            NavHome, NavPrograms, NavFlowChains, NavInstaller,
        ...
        public MainWindow()
        {
            ...
            UserNameText.Visibility = System.Windows.Visibility.Collapsed;
        }
        ...
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            // Only create the TrayService instance here; actual initialization (requiring a valid HWND)
            // is deferred to OnSourceInitialized.
            try
            {
                if (SettingsService.Instance.Settings.MinimizeToTray)
                    _trayService = new Coclico.Services.TrayService();
            }
        ...
        protected override void OnSourceInitialized(EventArgs e)
        {
            ...
            // Initialize tray here (OnSourceInitialized) because the HWND is now valid,
            // whereas in the constructor the handle is still IntPtr.Zero.
            try
            {
                if (SettingsService.Instance.Settings.MinimizeToTray && _trayService != null && !_trayService.IsInitialized)
                {
                    _trayService.Initialize(this);
                    _trayService.ShowBalloon(LocalizationService.Instance.Get("App_Title"), LocalizationService.Instance.Get("Tray_Running"));
                }
            }
        ...
        private void OnHotkeyTriggered(KeyboardShortcut shortcut)
        {
            try
            {
                if (shortcut.ActionType == "FlowChain" && !string.IsNullOrEmpty(shortcut.ChainId))
                {
                    var chains = new FlowChainService().GetFlowChains();
                    var chain = System.Linq.Enumerable.FirstOrDefault(chains, c => c.Id == shortcut.ChainId);
                    if (chain != null)
                    {
                        ToastService.Show($"▶ Flow Chain: {chain.Name}");
                        _ = new Services.FlowChainExecutionService().ExecuteChainAsync(chain);
                    }
                }
            }
        ...
        protected override void OnClosing(System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                if (SettingsService.Instance.Settings.MinimizeToTray)
                {
                    e.Cancel = true;
                    this.Hide();
                    try { _trayService?.ShowBalloon(LocalizationService.Instance.Get("App_Title"), LocalizationService.Instance.Get("Tray_Running")); }
        ...
        {
            bool newCompact = !SettingsService.Instance.Settings.CompactMode;
            SettingsService.Instance.Settings.CompactMode = newCompact;
        ...
        private async void NavItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            UIElement newContent = tag switch
            {
                ...
                "FlowChains"   => _flowChainsView ??= new FlowChainsView(),
            ...
            AiChatService.Instance.CurrentStatusContext = tag switch
            {
                ...
                "FlowChains" => "L'utilisateur est sur les Flow Chains (automatisation).",
        ...
        public void SetCompactMode(bool compact)
        {
            try
            {
                ...
                double to   = compact
                    ...
                    : Math.Clamp(SettingsService.Instance.Settings.SidebarWidth, 120, 400);
        ...
        private UIElement CreateErrorFallback(string message)
        {
            ...
            {
                Background = new System.Windows.Media.SolidColorBrush(System.Windows.Media.Color.FromArgb(255, 6, 6, 12)),
            ...
            stack.Children.Add(new System.Windows.Controls.Button
            {
                Content = "Réessayer",
                Width = 140,
                Margin = new Thickness(0, 14, 0, 0),
                Command = new RelayCommand(_ => MainContentFrame.Content = _dashboardView ??= new DashboardView())
            });
        ...
        private async void AiChatPanel_ActionRequested(object? sender, string action)
        {
            ...
            UIElement? target = action switch
            {
                ...
                "open_flowchains"  => _flowChainsView ??= new FlowChainsView(),
            ...
            AiChatService.Instance.CurrentStatusContext = action switch
            {
                ...
                _                 => AiChatService.Instance.CurrentStatusContext
            };
            ...
            var navButton = action switch
            {
                ...
                _                  => (System.Windows.Controls.RadioButton?)null
            };
        ...
        {
            _isChatOpen = open;
            var slideTransform = AiChatOverlay.RenderTransform as System.Windows.Media.TranslateTransform;
