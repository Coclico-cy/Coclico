...
public class CleaningViewModel : INotifyPropertyChanged
{
    ...
    public ObservableCollection<CleaningService.CleaningCategory> Categories { get; } = new();

    public CleaningViewModel()
    {
        _statusMessage = ServiceContainer.GetRequired<LocalizationService>().Get("Cleaning_Ready");
    ...
    private async Task LoadCategoriesAsync()
    {
        try
        {
            var cats = _cleaningService.GetAvailableCategories();
            foreach (var c in cats) Categories.Add(c);

            var bytes = await _cleaningService.EstimateCleanableBytesAsync();
    ...
    {
        var loc = ServiceContainer.GetRequired<LocalizationService>();
    ...
    public async Task StartDeepCleaningAsync()
    {
        ...
        try
        {
            var result = await _cleaningService.ExecuteDeepCleanAsync(selected, progressReporter, _cts.Token);
    ...
    private static string FormatBytes(long bytes)
    {
        ...
        return $"{size.ToString("F1", CultureInfo.CurrentCulture)} {sizes[order]}";
    }
