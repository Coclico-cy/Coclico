...
public class ScannerViewModel : INotifyPropertyChanged
{
    ...
    {
        get => _hasScanned;
        private set { _hasScanned = value; OnPropertyChanged(); }
    }
    ...
    {
        _statusMessage = ServiceContainer.GetRequired<LocalizationService>().Get("Scanner_Ready");
        if (string.IsNullOrEmpty(_statusMessage)) _statusMessage = "Prêt à analyser.";

        ScanCommand = new CommunityToolkit.Mvvm.Input.AsyncRelayCommand<object?>(async _ => await ScanApplicationsAsync());
    }
    ...
    private async Task ScanApplicationsAsync()
    {
        ...
        IsLoading = true;
        StatusMessage = ServiceContainer.GetRequired<LocalizationService>().Get("Scanner_Scanning") is { Length: > 0 } s ? s : "Analyse en cours…";

        try
        {
            var programsList = await ServiceContainer.GetRequired<InstalledProgramsService>().GetAllInstalledProgramsAsync();
            ...
            HasScanned = true;
            var doneKey = ServiceContainer.GetRequired<LocalizationService>().Get("Scanner_Done");
        ...
        catch (Exception ex)
        {
            ...
            var errKey = ServiceContainer.GetRequired<LocalizationService>().Get("Scanner_Error");
