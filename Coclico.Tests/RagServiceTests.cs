...
namespace Coclico.Tests
{
    public class RagServiceTests : IDisposable
    {
        ...
        {
            // No headers → fallback: whole content as one chunk (if length > 40)
        ...
        {
            // Content under the 40-char threshold per section should be skipped
            File.WriteAllText(Path.Combine(_tempDir, "short.md"),
                "## TinySection\nShort.");
            _svc.BuildIndex(_tempDir);
            // The section text "Short." is < 40 chars — skipped or counted as at most 1
        ...
        public void BuildIndex_IgnoresNonMarkdownFiles()
        {
            ...
            _svc.BuildIndex(_tempDir);
            Assert.Equal(0, _svc.ChunkCount); // .txt ignored
        }
        ...
        public void Search_UnrelatedQuery_ReturnsEmpty()
        {
            ...
            // Completely unrelated query with no token overlap
        ...
        public void Search_RespectTopKLimit()
        {
            ...
            // With topK=2, at most 2 sections separated by ---
