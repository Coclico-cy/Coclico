...
namespace Coclico.Tests
{
    ...
    public class SettingsSanitizationTests : IDisposable
    {
        ...
        public async Task SettingsService_SaveAsync_WritesJsonFile()
        {
            ...
            var ex = await Record.ExceptionAsync(() => new SettingsService().SaveAsync());
            Assert.Null(ex);
        ...
        {
            var ex = await Record.ExceptionAsync(() => new SettingsService().LoadAsync());
            Assert.Null(ex);
        }
        ...
        {
            var ex = Record.Exception(() => new SettingsService().Load());
            Assert.Null(ex);
        }
        ...
        {
            var ex = Record.Exception(() => new SettingsService().Save());
            Assert.Null(ex);
        }
        ...
        {
            var ex1 = Record.Exception(() => new SettingsService().EnableAutostart());
            var ex2 = Record.Exception(() => new SettingsService().DisableAutostart());
            Assert.Null(ex1);
            Assert.Null(ex2);
        }
        ...
        public void SettingsService_DisableAutostart_IsIdempotent()
        {
            ...
            var ex1 = Record.Exception(() => new SettingsService().DisableAutostart());
            var ex2 = Record.Exception(() => new SettingsService().DisableAutostart());
            Assert.Null(ex1);
            Assert.Null(ex2);
        }
