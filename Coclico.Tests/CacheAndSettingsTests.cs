...
namespace Coclico.Tests
{
    public class CacheAndSettingsTests
    {
        ...
        public async Task Cache_SetGet_Works()
        {
            ...
            await new CacheService().SetAsync(key, data, TimeSpan.FromMinutes(5));

            var read = await new CacheService().GetAsync<List<int>>(key);
            Assert.NotNull(read);
            Assert.Equal(3, read.Count);

            try { await new CacheService().InvalidateAsync(key); } catch { }
        }
