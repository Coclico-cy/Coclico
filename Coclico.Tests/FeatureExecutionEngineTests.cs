...
namespace Coclico.Tests
{
    public class FeatureExecutionEngineTests
    {
        ...
        public async Task RunFeatureAsync_SetsSuccessState()
        {
            ...
            var engine = new FeatureExecutionEngine();
        ...
        [Fact]
        public async Task WorkflowService_IntegratesFeatureEngine()
        {
            var svc = new WorkflowService();
            var chain = new Models.WorkflowPipeline { Name = "FeatureEngineChainTest" };
            chain.Items.Add(new Models.PipelineStep { NodeType = Models.NodeType.Start, Name = "Start", IsEnabled = true });
            chain.Items.Add(new Models.PipelineStep { NodeType = Models.NodeType.Delay, Name = "Delay", IsEnabled = true, DelaySeconds = 0 });
            chain.Items.Add(new Models.PipelineStep { NodeType = Models.NodeType.End, Name = "End", IsEnabled = true });

            var result = await svc.ExecuteChainAsync(chain);

            Assert.True(result.Success);
            var state = svc.Engine.GetState($"WorkflowPipeline:{chain.Name}");
            Assert.NotNull(state);
            Assert.Equal(FeatureExecutionStatus.Success, state!.Status);
