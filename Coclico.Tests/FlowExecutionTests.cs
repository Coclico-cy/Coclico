...
{
    public class FlowChainServiceTests
    {
        [Fact]
        public void GetFlowChains_ReturnsNonNull()
        {
            var svc = new FlowChainService();
            var chains = svc.GetFlowChains();
            Assert.NotNull(chains);
        }

        [Fact]
        public void GetFlowChains_ReturnsObservableCollection()
        {
            var svc = new FlowChainService();
            var chains = svc.GetFlowChains();
            Assert.IsAssignableFrom<ObservableCollection<FlowChain>>(chains);
        }

        [Fact]
        public void SaveAndGetFlowChains_RoundTrip()
        {
            var svc = new FlowChainService();
            var original = svc.GetFlowChains();
            var testName = "UnitTestChain_" + Guid.NewGuid().ToString("N")[..6];

            var testChain = new FlowChain { Name = testName };
            original.Add(testChain);

            svc.SaveFlowChains(original);

            var reloaded = svc.GetFlowChains();
            Assert.Contains(reloaded, c => c.Name == testName);

            reloaded.Remove(reloaded[reloaded.Count - 1]);
            svc.SaveFlowChains(reloaded);
        }

        [Fact]
        public void SaveFlowChains_Empty_DoesNotThrow()
        {
            var svc = new FlowChainService();
            var ex = Record.Exception(() =>
                svc.SaveFlowChains(new ObservableCollection<FlowChain>()));
            Assert.Null(ex);
        }
    }

    public class FlowChainExecutionServiceTests
    {
        private static FlowChain MakeChain(params NodeType[] types)
        {
            var chain = new FlowChain { Name = "Test" };
            foreach (var t in types)
                chain.Items.Add(new FlowItem { NodeType = t, Name = t.ToString(), IsEnabled = true });
            return chain;
        }
        ...
        {
            var svc = new FlowChainExecutionService();
            var chain = new FlowChain { Name = "Empty" };
        ...
        {
            var svc = new FlowChainExecutionService();
            var chain = new FlowChain { Name = "DelayTest" };
            chain.Items.Add(new FlowItem { NodeType = NodeType.Start, Name = "Start", IsEnabled = true });
            chain.Items.Add(new FlowItem { NodeType = NodeType.Delay, Name = "Wait", IsEnabled = true, DelaySeconds = 0 });
            chain.Items.Add(new FlowItem { NodeType = NodeType.End, Name = "End", IsEnabled = true });
        ...
        {
            var svc = new FlowChainExecutionService();
            var chain = new FlowChain { Name = "DisabledTest" };
            chain.Items.Add(new FlowItem { NodeType = NodeType.Start, Name = "Start", IsEnabled = true });
            chain.Items.Add(new FlowItem { NodeType = NodeType.Delay, Name = "Disabled", IsEnabled = false });
            chain.Items.Add(new FlowItem { NodeType = NodeType.End, Name = "End", IsEnabled = true });
        ...
        {
            var svc = new FlowChainExecutionService();
            var chain = new FlowChain { Name = "EventTest" };
            chain.Items.Add(new FlowItem { NodeType = NodeType.Delay, Name = "Delay", IsEnabled = true, DelaySeconds = 0 });
    ...
    public class ExecutionResultTests
    {
        ...
        {
            var result = new FlowChainExecutionService.ExecutionResult();
